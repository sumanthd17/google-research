...
flags.DEFINE_string(
    "wikinews_archive",
    None,
    "Glob pattern matching .bz2 files containing jsonl as output by " "wikiextractor.",
)
flags.DEFINE_string("output_dir_wiki", None, "Output directory for wikidoc files.")
flags.DEFINE_string(
    "output_dir_text", None, "Output directory for clean text output files."
)
flags.DEFINE_string(
    "doc_index_path",
    None,
...
def extract_text_and_mentions(doc_index, wiki_doc_parser, wiki_dir, text_dir):
    ...
    mention_index_df = pd.concat(mention_index_dfs, ignore_index=True,).sort_values(
        by=["docid", "position"],
        ignore_index=True,
    )
...
def verify_mention_index(original, reconstructed):
    ...
    def df_to_strings(df):
        ...
        return df.apply(
            lambda r: "{}:{}-{}".format(r["docid"], r["mention"], r["position"]), axis=1
        ).values
...
def verify_doc_index(original, reconstructed):
    ...
    merged = pd.merge(
        original, reconstructed, how="left", on="docid", suffixes=("_expected", "_got")
    )
    text_diff = merged["text_md5_expected"] != merged["text_md5_got"]
    for missed in merged[text_diff].itertuples():
        logging.warning(
            "Text hash mismatch: %s - %s", missed.Index, missed.text_md5_got
        )
...
def add_text_hash_column(docid_to_text_hash, doc_index):
    doc_index["text_md5"] = doc_index.index.to_series().map(docid_to_text_hash.get)
...
def main(argv):
    ...
    archive_parser = common.WikiNewsArchiveParser(FLAGS.wikinews_archive, language)
    archive_consumer = common.WikiNewsArchiveConsumer(
        archive_parser, FLAGS.output_dir_wiki, FLAGS.max_docs, FLAGS.max_docs
    )
    ...
    if FLAGS.mode == "raw":
        ...
        mention_index, docid_to_text_hash = extract_text_and_mentions(
            doc_index, wiki_doc_parser, FLAGS.output_dir_wiki, FLAGS.output_dir_text
        )
        ...
        logging.info(
            "Extracted potential contexts for %d mentions.",
            fresh_mention_index.shape[0],
        )

        if (
            verify_doc_index(doc_index, fresh_doc_index)
            and verify_mention_index(mention_index, fresh_mention_index)
            and verify_context(mention_index, FLAGS.output_dir_text)
        ):
            print(
            ...
            print(
                "Fail! Extracted mentions and contexts do not match all "
