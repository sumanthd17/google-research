...
from . import html_anchor_parser
...
def wiki_encode(url):
    ...
    return urllib.parse.quote(url.replace(" ", "_"), ";@$!*(),/:'").replace("%7E", "~")
    ...
    def extract_docs(self, doc_index_df=None):
        ...
        for i, item in enumerate(self._archive_parser.documents()):
            ...
            if doc_index_df is None or docid in doc_index_df.index:
                ...
                if (
                    self._max_docs
                    and self._max_docs > 0
                    and len(doc_index_list) >= self._max_docs
                ):
...
class WikiNewsDocParser(object):
    ...
    def parse_doc(self, wiki_doc, docid):
        ...
        try:
            ...
            logging.warning(
                "Ignore %d mentions due to parse fail [%s]", len(raw_mentions), docid
            )
            raw_mentions = []
...
def load_doc_index(doc_index_path):
    ...
    return pd.read_csv(doc_index_path, sep="\t", encoding="utf-8", index_col="docid")
...
def write_doc_index(doc_index, doc_index_path, overwrite=False):
    ...
    doc_index.to_csv(doc_index_path, sep="\t", encoding="utf-8", columns=column_order)
